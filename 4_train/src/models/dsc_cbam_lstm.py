"""
DSC-CBAM-LSTM Model Components
"""
...
class DSC_CBAM_LSTM(nn.Module):
    """DSC-CBAM-LSTM Model for Network Threat Detection
    
    Architecture:
        Input(18) → Conv1D(32) → DSC(64) → CBAM → BiLSTM(64) → FC(4)
    ...
    def __init__(self, input_dim=18, num_classes=4, hidden_dim=64, bidirectional=True, dropout=0.3):
        super().__init__()
        out_dim = hidden_dim * (2 if bidirectional else 1)

        self.conv = nn.Conv1d(input_dim, 32, kernel_size=1)
        self.dsc = DepthwiseSeparableConv(32, 64)
        self.cbam = CBAM(64)
        self.lstm = nn.LSTM(
            64, hidden_dim,
            num_layers=1, 
