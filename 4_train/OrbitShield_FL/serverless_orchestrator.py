...
from .topology_ns3 import load_all_ns3_topology_snapshots
from .transfer_scheduler import can_transfer_model, model_state_size_bytes
...
class ServerlessOrchestrator:
    ...
    def __init__(self, config: FederatedConfig) -> None:
        ...
        if config.max_samples:
            self.dataset = self._subsample_dataset(self.dataset, config.max_samples, config.seed)
        train_x, train_y = self.dataset["train"]
        self.partition_map = partition_train_dataset_for_satellites(
            train_npz_path=None if config.max_samples else os.path.join(config.data_dir, "train.npz"),
        ...
        self.fast_val_loader = self._build_eval_loader(self.dataset["val"], config.eval_subset_size, config.seed + 101)
        self.fast_test_loader = self._build_eval_loader(self.dataset["test"], config.test_subset_size, config.seed + 202)
        ...
        self.ns3_trace_bundle = None
        self.ns3_topology_snapshots: dict[int, dict[str, object]] = {}
        if self.config.topology_backend == "ns3":
            self.ns3_trace_bundle, self.ns3_topology_snapshots = load_all_ns3_topology_snapshots(self.config.ns3_trace_dir)
    ...
    def _subsample_dataset(
        ...
        """Subsample each split to a bounded number of samples for faster experiments."""
        rng = np.random.default_rng(seed)
        reduced: dict[str, tuple[np.ndarray, np.ndarray]] = {}
        for split_name, (features, labels) in dataset.items():
            limit = min(max_samples, len(features))
            indices = rng.choice(len(features), size=limit, replace=False)
            reduced[split_name] = (features[indices], labels[indices])
        return reduced
    ...
    def _build_eval_loader(
        ...
        """Create an evaluation loader, optionally using a fixed random subset."""
        features, labels = split
        if subset_size is not None and subset_size < len(features):
            rng = np.random.default_rng(seed)
            indices = rng.choice(len(features), size=subset_size, replace=False)
            features = features[indices]
            labels = labels[indices]
        return make_eval_loader(features, labels, batch_size=self.config.batch_size)
    ...
    def _evaluate_loss_from_weights(self, weights: StateDict) -> float:
        ...
        metrics = evaluate_global_model(model, self.fast_val_loader, self.device)
        return float(metrics["loss"])

    def _evaluate_round_metrics(
        self,
        round_idx: int,
        communication_bytes: int,
        stale_contributors: int,
        total_attempted_uploads: int,
        failed_uploads: int,
        failed_inter_links: int,
        total_inter_attempts: int,
    ) -> dict[str, object]:
        """Evaluate validation/test metrics for one round using configured cadence."""
        val_metrics = evaluate_global_model(self.global_model, self.fast_val_loader, self.device)
        test_metrics = None
        if self.config.test_every > 0 and round_idx % self.config.test_every == 0:
            test_metrics = evaluate_global_model(self.global_model, self.fast_test_loader, self.device)
    ...
    def _get_topology_snapshot(self, round_idx: int) -> dict[str, object]:
        """Resolve the topology snapshot for one training round."""
        if self.config.topology_backend == "ns3":
            if round_idx not in self.ns3_topology_snapshots:
                raise IndexError(f"ns-3 topology trace does not contain round {round_idx}")
            return self.ns3_topology_snapshots[round_idx]
        return generate_topology_snapshot(
    ...
    def train_one_federated_round(self, round_idx: int) -> dict[str, object]:
        ...
        topology = self._get_topology_snapshot(round_idx)
        ...
        for client_id, client in self.clients.items():
            ...
            if self.config.topology_backend == "ns3":
                ...
                if not can_transfer_model(
                    ...
                    link_state=intra_link,
                    bandwidth_mbps=intra_bandwidth,
                ):
                    failed_uploads += 1
                    continue
            ...
            for neighbor in neighbors:
                ...
                if self.config.topology_backend == "ns3":
                    ...
                    model_can_transfer = can_transfer_model(
                        ...
                        bandwidth_mbps=inter_bandwidth,
                    )
                if link_state.success and model_can_transfer:
    ...
    def run_federated_training(self) -> dict[str, object]:
        ...
        for round_idx in range(1, self.config.rounds + 1):
            ...
            test_repr = f"{metrics['test_accuracy']:.4f}" if not np.isnan(metrics["test_accuracy"]) else "skip"
            print(
                f"Round {round_idx:02d} | "
                f"val_acc={metrics['val_accuracy']:.4f} "
                f"test_acc={test_repr} "
