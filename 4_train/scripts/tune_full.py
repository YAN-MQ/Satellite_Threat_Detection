...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune DSC-CBAM-GRU on the selected dataset")
    parser.add_argument("--data_dir", default="../dataset_cicids17")
    parser.add_argument("--output_dir", default="experiments/tuning")
...
def main() -> None:
    ...
    for idx, (hidden_dim, bidirectional, dropout, lr, weight_decay) in enumerate(search_space, start=1):
        ...
        row = flatten_metrics(name, "tuning", params, flops, result)
        row.update(
            {
                "hidden_dim": hidden_dim,
                "bidirectional": bidirectional,
        ...
        summary_path = os.path.join(args.output_dir, "tuning_summary.csv")
        detail_path = os.path.join(args.output_dir, "tuning_results.json")
        write_csv(
            summary_path,
            rows,
            [
