...
def main():
    # 创建日志记录器
    experiment_name = f"centralized_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    logger = TrainingLogger(log_dir='logs', experiment_name=experiment_name)
    results_writer = ResultsWriter(results_dir='results')
    
    # 记录配置
    logger.log_config(config)
    logger.info(f"Starting centralized training experiment: {experiment_name}")
